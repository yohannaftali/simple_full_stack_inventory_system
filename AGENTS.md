...
| #80 | feat(frontend): restrict Troubleshooting page to desktop/mobile (local data only); auto-clear logs on logout | **closed 2026-08-04** (commit 71b5316, pushed to main) - page-visibility gate + credential redaction verified live; `utils/app_logger.py` redesigned to per-client-session log buckets (`storage/data/sessions/<client_id>.log`); `on_logout_click`'s `clear_logs(client_id)` now runs on every platform. The logout-then-Show-Logs round trip itself was not exercised live (no valid login credentials available in that session) - worth a real click-through next time credentials are available | 2026-08-04 |
