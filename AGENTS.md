...
| #80 | feat(frontend): restrict Troubleshooting page to desktop/mobile (local data only); auto-clear logs on logout | ready-for-review (page-visibility gate + credential redaction verified live; `utils/app_logger.py` redesigned to bucket logs per-client-session (`storage/data/sessions/<client_id>.log`, mirrors the existing `_ServerFileStore` convention) instead of one process-global buffer/file - confirmed live via `docker exec` that separate browser sessions now write to separate log files; `on_logout_click`'s `clear_logs(client_id)` now runs on every platform, not just desktop/mobile, since it can no longer touch another session's data; the logout-then-Show-Logs round trip itself still not exercised live - needs a real login with valid credentials to confirm end to end) | 2026-08-04 |
