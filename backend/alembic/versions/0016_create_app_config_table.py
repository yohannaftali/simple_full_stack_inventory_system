"""create app_configs table, seed its one default row, seed the
master_config module, and grant it to the default superuser
...
GROUP_NAME = "Application Configuration"

# (name, label, icon, description, sort)
DEFAULT_MODULES = [
    ("master_config", "Application Config", "settings", "Configure application-wide settings", 4),
]
...
_app_configs_table = sa.table(
    "app_configs",
    sa.column("id", sa.Integer),
    sa.column("app_title", sa.String),
    sa.column("footer", sa.String),
)
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("app_title", sa.String(length=100), nullable=False, server_default="SFSIS"),
        sa.Column("footer", sa.String(length=255), nullable=False, server_default=""),
    ...
    existing = bind.execute(sa.select(_app_configs_table.c.id)).first()
    if existing is None:
        bind.execute(_app_configs_table.insert().values(app_title="SFSIS", footer=""))
