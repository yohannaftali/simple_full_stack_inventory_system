"""seed default modules and grant them to the default superuser
...
DEFAULT_MODULES = [
    ("ap_module", "Modules", "widgets", "Manage application modules", 1),
    ("ap_master_user", "Users", "people", "Manage users", 2),
    ("master_location", "Locations", "place", "Manage stock locations", 10),
    ("master_material", "Materials", "category", "Manage materials", 11),
    ("stock_in", "Stock In", "call_received", "Receive stock", 20),
    ("stock_out", "Stock Out", "call_made", "Issue stock", 21),
    ("stock_browse", "Stock Browse", "inventory", "Browse current stock", 22),
]
...
def upgrade() -> None:
    ...
    for name, label, icon, description, sort in DEFAULT_MODULES:
        ...
        if module_id is None:
            ...
            # sa.table()'s lightweight column() doesn't declare a primary key,
            # so inserted_primary_key can't resolve it - look the id up by
            # name (unique) instead, portable across dialects.
            module_id = bind.execute(
                sa.select(_modules_table.c.id).where(_modules_table.c.name == name)
            ).scalar()
